--  Bowyer_Watson — Ada 2023 educational package for the Bowyer–Watson
--  algorithm: incremental 2-D Delaunay triangulation of a finite point set.
--  Insert sites one by one into a valid triangulation; delete triangles
--  whose circumcircles contain the new site (leaving a star-shaped hole);
--  retriangulate the hole to the new site; finally strip triangles that
--  touch a super-triangle vertex. Dual of the Voronoi diagram.
--  Primary source:
--  https://en.wikipedia.org/wiki/Bowyer–Watson_algorithm
--  Sibling packages (README only; do not `with`):
--    Ada-Fortunes-Algorithm, Ada-Voronoi-Diagram, Ada-Delaunay-Triangulation
--    (survey) — planned ahead in the RobertBoettcherSF series.
...
is
   ...
   --  Hole boundary edges during one insertion (star-shaped polygon).
   ...
   function Has_Near_Duplicate
     ...
     with Pre => Tol >= 0.0, Global => null;
   --  True iff some pair of distinct sites is within Tol (L∞ or Euclidean
   --  via Dist2 ≤ Tol²). Used to raise Invalid_Argument before triangulate.
   ...
   function Triangulate (Points : Point_Array) return Triangulation
     with Global => null;
   --  Compute a 2-D Delaunay triangulation of Points via Bowyer–Watson.
   --  Requires Points'Length in 3 .. Max_Points and no near-duplicates;
   --  otherwise raises Invalid_Argument. Returns triangles whose vertices
   --  are indices into Points (1-based relative to Points'First mapped to 1
   --  in the result — see Triangulate_Indexed for explicit mapping).
   --
   --  Steps (Wikipedia incremental description):
   ...
   ---------------------------------------------------------------------------
   -- Delaunay verification helpers (educational)
   ---------------------------------------------------------------------------

   function Is_Delaunay_Edge_Empty
     (Points : Point_Array; T : Triangulation) return Boolean
     with Global => null;
   --  For every triangle Tri and every site P not a vertex of Tri, check
   --  that not In_Circumcircle (Tri, P). True ⇒ empty-circle property holds
   --  under the educational Float predicates (may fail on near-cocircular
   --  or near-collinear classroom degeneracies).
